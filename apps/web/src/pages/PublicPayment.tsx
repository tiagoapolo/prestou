...
export function PublicPaymentPage() {
  ...
  async function copyCode() {
    ...
    try {
      try {
        ...
        const area = document.createElement("textarea"); area.value = payment.brCode; document.body.append(area); area.select();
  ...
  async function confirm() {
    ...
    try {
      ...
      setDone(true);
    } catch (e) { setError(userMessage(e, "Não foi possível enviar a confirmação. Tente novamente.")); }
  ...
  return <main className="public-page">
    <Card className="receipt-card">
      ...
      {(copied || showQr) && <section className="confirm-panel"><div className="step-mark">2</div><h2>Já fez o Pix?</h2><p>Avise o prestador para ele conferir. O comprovante é opcional.</p><label className="file-label">{file ? file.name : "Anexar comprovante (opcional)"}<Input type="file" accept="image/jpeg,image/png,image/webp,image/heic,application/pdf" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /></label>{error && <ErrorNotice message={error} />}<Button loading={busy} loadingLabel="Enviando…" onClick={confirm}>Já paguei</Button></section>}
